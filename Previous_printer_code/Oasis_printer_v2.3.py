...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.clean_interval = 5                                         # layers between mid-print cleans (0 = disabled)
        self.mid_print_clean_sequence = [("preheat", 3), ("prime", 3)] # short clean sequence used during print
        ...
        # Mid-print clean interval spinbox (added to status bar; 0 = disabled)
        self._clean_interval_label = QtWidgets.QLabel("Clean every:")
        self._clean_interval_spin = QtWidgets.QSpinBox()
        self._clean_interval_spin.setRange(0, 50)
        self._clean_interval_spin.setValue(self.clean_interval)
        self._clean_interval_spin.setSuffix(" layers")
        self._clean_interval_spin.setToolTip("Mid-print printhead clean interval (0 = disabled)")
        self._clean_interval_spin.valueChanged.connect(
            lambda v: setattr(self, "clean_interval", v)
        )
        self.form.statusBar.addPermanentWidget(self._clean_interval_label)
        self.form.statusBar.addPermanentWidget(self._clean_interval_spin)
    ...
    def _MaybeCleanMidPrint(self) -> None:
        """Run a short printhead clean if the clean interval is due.

        Called once per layer inside _PrintSVG_inner after current_layer increments.
        Skips layer 0, skips if clean_interval == 0 (disabled), and skips if
        inkjet is not connected. Runs synchronously so the print loop waits.
        """
