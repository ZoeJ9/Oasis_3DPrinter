...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _AutoHeadCleanWorker(self):
        """Background worker for automatic per-layer head cleaning after _Printed capture.
        Sequence: (Preheat x10 + Prime x10) x2, 1000 ms interval between each burst.
        """
        preheat_pulses = getattr(self, "preheat_pulses", 5000)
        prime_pulses = getattr(self, "prime_pulses", 100)
        sequence = [
        ...
        print("AutoHeadClean: starting per-layer cleaning sequence")
        for action, count in sequence:
            for _ in range(count):
                if action == "preheat":
                    self.inkjet.Preheat(preheat_pulses)
                ...
                print(f"AutoHeadClean: {done}/{total} ({action})")
                time.sleep(1.0)
        print("AutoHeadClean: sequence complete")
                ...
                if self.current_layer >= self.layers:
                    ...
                    while True:
                        ...
                        if (
                            ...
                            break
                    if hasattr(self, "camera_window") and self.camera_window.camera_enabled:
                        # Start head clean and camera capture in parallel
                        clean_thread = threading.Thread(target=self._AutoHeadCleanWorker, daemon=True)
                        clean_thread.start()
                        self.camera_window.capture_sync(
                ...
                if hasattr(self, "camera_window") and self.camera_window.camera_enabled:
                    ...
                    while True:
                        ...
                        if (
                            ...
                            break
                    # Start head clean and camera capture in parallel, wait for both before NewLayer()
                    clean_thread = threading.Thread(target=self._AutoHeadCleanWorker, daemon=True)
                    clean_thread.start()
                    self.camera_window.capture_sync(
                        f"Layer_{self.current_layer - 1:03d}_Printed"
