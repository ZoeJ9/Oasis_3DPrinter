...
WARMUP_FRAMES         = 15             # increased to ensure exposure settles before capture
...
class CameraController(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        # Capture exposure spinbox (-7 to -1, log2 seconds)
        ...
        self.exposure_spin.setToolTip("Camera exposure for captures (log2 seconds). -3 = 1/8s, -6 = 1/64s")
        self.exposure_spin.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Fixed)
    ...
    def capture_sync(self, filename_suffix):
        ...
        try:
            ...
            # 5. Collect frames then reject outliers by mean brightness
            raw_frames = []
            for _ in range(max(1, AVERAGING_FRAMES)):
                ret, f = cap.read()
                if ret:
                    raw_frames.append(f.astype(np.float32))
            cap.release()
            ...
            # Reject frames whose mean brightness deviates >10% from the median
            means = np.array([f.mean() for f in raw_frames])
            median = np.median(means)
            frames = [f for f, m in zip(raw_frames, means)
                      if abs(m - median) / (median + 1e-6) < 0.10]
            if not frames:
                frames = raw_frames  # fallback: keep all if everything was rejected
            print(f"CAMERA: {len(frames)}/{len(raw_frames)} frames kept after outlier rejection")
            ...
            # 7. Crop to centre 80% to discard blurry edges
            h_full, w_full = frame.shape[:2]
            h_crop = int(h_full * 0.8)
            w_crop = int(w_full * 0.8)
            y0 = (h_full - h_crop) // 2
            x0 = (w_full - w_crop) // 2
            frame = frame[y0:y0 + h_crop, x0:x0 + w_crop]
            print(f"CAMERA: Cropped to centre 80%: {w_crop}x{h_crop}")

            # 8. Save (PNG)
            filename = f"{filename_suffix}.png"
            filepath = os.path.join(self.output_dir, filename)
            cv2.imwrite(filepath, frame)
            print(f"CAMERA: Saved {filepath}  ({w_crop}x{h_crop}, avg={len(frames)})")
