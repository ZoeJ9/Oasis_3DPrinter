...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.config_mode_active = False  # True during a DOE Print run
    ...
    def RunDOEPrint(self):
        """Phase 2: print the currently loaded SVG (cube/lattice) using the
        hardcoded DOE_CONDITIONS[SELECTED_RUN] condition instead of the SVG's
        own implied thickness. Runs the normal PrintSVG thread — DOE injection
        happens once, right after _init_print_state, inside _PrintSVG_inner.
        """
        from config_runner import DOE_CONDITIONS, SELECTED_RUN
        if self.file_loaded != 2:
            QtWidgets.QMessageBox.warning(
        ...
        if self.printing_state != 0:
            ...
            return
        if SELECTED_RUN not in DOE_CONDITIONS:
            QtWidgets.QMessageBox.critical(
                self.ui, "DOE Config Error",
                f"SELECTED_RUN={SELECTED_RUN} not in DOE_CONDITIONS."
            )
            return
        self.config_mode_active = True
        self._doe_selected_run  = SELECTED_RUN
        self._printing_stop_event = threading.Event()
        self.printing_thread = threading.Thread(target=self.PrintSVG, daemon=True)
        self.printing_thread.start()
    ...
    def _PrintSVG_inner(self):
        ...
        if (
            ...
            # DOE mode: apply_print_condition must run AFTER _init_print_state,
            # which hardcodes print_speed=2200.0 — calling before would let
            # that default clobber the injected DOE value.
            if getattr(self, "config_mode_active", False):
                from config_runner import apply_print_condition, DOE_CONDITIONS, DOE_LAYER_THICK_MM
                params = dict(DOE_CONDITIONS[self._doe_selected_run])
                params["layer_thickness"] = DOE_LAYER_THICK_MM
                ...
                for key, want in (
                    ...
                    if want is not None and applied[key] != (int(want) if key in ("applied_layer_passes", "applied_density") else float(want)):
                        raise RuntimeError(
                            f"DOE injection mismatch: {key} readback {applied[key]!r} "
                            f"!= requested {want!r}. Refusing to start print."
                        )
                print(f"[DOE Print] run {self._doe_selected_run}: {applied}")
