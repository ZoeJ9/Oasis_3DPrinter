...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _run_calibration_inner(self):
        ...
        # 2. Build calibration image_array directly — a filled circle in GRBL mm space.
        # The calibration SVG is not Slic3r format so ImageConverter cannot parse its layers.
        # We rasterise the circle here using the same DPI and bed constants as a normal print.
        ...
        _arr_w = int(BED_DIAMETER_MM * _px_per_mm) + 1
        _arr_h = int(BED_DIAMETER_MM * _px_per_mm) + 1
        calib_array = zeros((_arr_h, _arr_w))
        _cx_px = BUILD_CENTER_X_MM * _px_per_mm
        _cy_px = BUILD_CENTER_Y_MM * _px_per_mm
        _r_outer = (BED_RADIUS_MM + CALIB_LINE_WIDTH_MM / 2) * _px_per_mm
        _r_inner = (BED_RADIUS_MM - CALIB_LINE_WIDTH_MM / 2) * _px_per_mm
        for _row in range(_arr_h):
            for _col in range(_arr_w):
                _d = ((_row - _cy_px) ** 2 + (_col - _cx_px) ** 2) ** 0.5
                if _r_inner <= _d <= _r_outer:
                    calib_array[_row][_col] = 1
        ...
        self.svg_offset_y = BED_DIAMETER_MM / 2
        self.svg_offset_x = BED_DIAMETER_MM / 2
