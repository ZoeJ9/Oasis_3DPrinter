...
class HP45(serial.Serial):
    def __init__(self):
        ...
        input_log_path = os.path.join(LOGS_DIR, "Serial_input_log.txt")
        output_log_path = os.path.join(LOGS_DIR, "Serial_output_log.txt")
        self.Serial_input_file = open(input_log_path,"w") 
        self.Serial_input_file.close()
        self.Serial_input_file = open(input_log_path,"r+") 
        self.Serial_input_file.truncate(0)
        self.Serial_input_file.write("Serial input log file") 
        self.Serial_input_file.close() 
        
        self.Serial_output_file = open(output_log_path,"w") 
        self.Serial_output_file.close()
        self.Serial_output_file = open(output_log_path,"r+") 
    ...
    def Connect(self, serial_port):
        ...
        if (self.connection_state == 0): #if not yet connected
            ...
            if (self.temp_com_success == 1):
                ...
                self.status_thread = threading.Thread(target=self.GetStatus, daemon=True)
        ...
        while not self._stop_event.is_set():
            ...
            try: #attempt to read serial
                if (self.ser.in_waiting > 0):
                    ...
                    temp_read = str(temp_read.decode('utf-8', errors='ignore'))
                    temp_success = 1
            except Exception as e:
                print(f"Read error (continuing): {e}")
                read_buffer = ""  # 깨진 데이터 제거
                time.sleep(0.05)
                continue
            if temp_success == 1: 
