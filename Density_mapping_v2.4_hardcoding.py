...
from config_runner import ConfigRunner, apply_print_condition
from fisheye_apply import FisheyeCorrector

# ============================================================
# EXPERIMENT CONDITION — edit these before running, then start
# this file directly. "DOE Print" uses these values as-is; no
# CSV, no DOE_CONDITIONS dict, no run-number lookup.
# ============================================================
EXPERIMENT_DENSITY         = 250    # 0-1000
EXPERIMENT_LAYER_PASSES    = 3      # sweeps per layer before Z advances
EXPERIMENT_OVERFEED        = 3.5    # nl_piston_overfeed ratio (NOT the GUI slider's percent)
EXPERIMENT_SPREAD_SPEED    = 6000   # mm/min, spreader pass speed (grbl.nl_feed_speed)
EXPERIMENT_PRINT_SPEED     = 2200   # mm/min, inkjet sweep speed
EXPERIMENT_LAYER_THICKNESS = 0.1    # mm, spread thickness per layer

# Reference table — the 12-run DOE ladder these values were pulled from.
# Copy a row's values into the EXPERIMENT_* constants above to select a run.
#
# --- density ladder (passes=3 fixed, all else baseline) ---
# 1:  density=100, layer_passes=3, overfeed=3.5, spread_speed=6000, print_speed=2200
# 2:  density=175, layer_passes=3, overfeed=3.5, spread_speed=6000, print_speed=2200
# 3:  density=250, layer_passes=3, overfeed=3.5, spread_speed=6000, print_speed=2200   # BASELINE
# 4:  density=325, layer_passes=3, overfeed=3.5, spread_speed=6000, print_speed=2200
# 5:  density=400, layer_passes=3, overfeed=3.5, spread_speed=6000, print_speed=2200
#
# --- layer_passes ladder (density=250 fixed) ---
# 6:  density=250, layer_passes=1, overfeed=3.5, spread_speed=6000, print_speed=2200
# 7:  density=250, layer_passes=2, overfeed=3.5, spread_speed=6000, print_speed=2200
#     layer_passes=3 @ density=250 == run 3 (baseline)
# 8:  density=250, layer_passes=5, overfeed=3.5, spread_speed=6000, print_speed=2200
#
# --- packing ladder (S knobs at baseline) ---
# 9:  density=250, layer_passes=3, overfeed=2.0, spread_speed=9000, print_speed=2200
# 10: density=250, layer_passes=3, overfeed=5.0, spread_speed=3000, print_speed=2200
#
# --- dwell ladder ---
# 11: density=250, layer_passes=3, overfeed=3.5, spread_speed=6000, print_speed=800
# 12: density=250, layer_passes=3, overfeed=3.5, spread_speed=6000, print_speed=3000
# ============================================================
...
LED_FLUSH_FRAMES = 8    # cap.grab() count before retrieve to flush stale frames
...
class CameraController(QtWidgets.QWidget):
    ...
    def _led_on(self, n: int, wait_ack: bool = False, ack_timeout: float = 1.0) -> bool:
        """Turn on LED n (1-indexed).

        If wait_ack, block until the Arduino echoes back "LED n ON"
        (firmware prints ">>> LED n ON  (pin x)" right after digitalWrite),
        so callers know the LED is physically on before proceeding.
        Returns True if acked (or wait_ack is False and command was sent),
        False if not connected or the ack timed out.
        """
        with self._arduino_lock:
            if not (self._arduino_conn and self._arduino_conn.is_open):
                print(f"CAMERA: Arduino not connected, LED{n} skipped")
                return False
            if not wait_ack:
                self._arduino_conn.write(str(n).encode())
                return True

            self._arduino_conn.reset_input_buffer()
            self._arduino_conn.write(str(n).encode())
            expected = f"LED {n} ON"
            deadline = time.time() + ack_timeout
            while time.time() < deadline:
                try:
                    line = self._arduino_conn.readline().decode(errors="ignore").strip()
                except Exception:
                    break
                if line:
                    print(f"CAMERA: Arduino: {line}")
                if expected in line:
                    return True
            print(f"CAMERA: Arduino ACK timeout for LED{n}")
            return False
    ...
    def capture_led_sequence(self, filename_stem: str):
        ...
        try:
            ...
            for led_n in led_indices:
                ...
                print(f"CAMERA: LED{led_n}  mean = {frame.mean():.1f}")
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.config_mode_active = False  # True during a DOE Print run (uses EXPERIMENT_* constants)
        ...
        # Config-runner: CSV-driven parameter sweep (Phase 1: screening)
        ...
        # DOE Print: hardcoded single-condition full build (Phase 2: characterization)
        self.doe_print_btn = QtWidgets.QPushButton("DOE Print")
        self.doe_print_btn.clicked.connect(self.RunDOEPrint)
        self.form.statusBar.addPermanentWidget(self.doe_print_btn)
    ...
    def RunDOEPrint(self):
        """Print the currently loaded SVG using the hardcoded EXPERIMENT_*
        constants at the top of this file instead of the SVG's own implied
        thickness. Runs the normal PrintSVG thread — injection happens once,
        right after _init_print_state, inside _PrintSVG_inner.
        """
        if self.file_loaded != 2:
            QtWidgets.QMessageBox.warning(
                self.ui, "No SVG loaded",
                "Load an SVG file before starting a DOE print."
            )
            return
    ...
    def _run_config_thread(self, runner: "ConfigRunner") -> None:
        try:
            ...
            # Apply step-1 params now so config_layer_thickness/overfeed/etc. are
            # live before the initial spread. runner.run() re-applies the same
            # step-1 params as its first loop iteration — idempotent, harmless.
            runner._apply_step(runner._steps[0])

            # Spread first layer only when starting from layer 1.
            if start_layer == 1:
                print("--- Spreading initial powder layer ---")
                first_thickness = getattr(self, "config_layer_thickness", None) \
                    if getattr(self, "_active_config_runner", None) is not None \
                    else self.imageconverter.svg_layer_height[0]
                self.grbl.NewLayer(first_thickness)
    ...
    def _print_single_config_layer(self, step: dict, layer_idx: int) -> None:
        ...
        if self.current_layer <= self.imageconverter.svg_layers:
            ...
            if getattr(self, "_active_config_runner", None) is not None:
                # Config mode: spread thickness comes from the CSV, not the SVG.
                # current_layer_height stays SVG-derived so progress/remaining-height
                # displays keep reading the file's own layer index.
                self._pending_layer_thickness = self.config_layer_thickness
            else:
    ...
    def PrintSVG(self):
        ...
        try:
            ...
            # Reset so a plain "Print" run after a DOE run isn't affected.
            self.config_mode_active = False
    ...
    def _PrintSVG_inner(self):
        ...
        if (
            ...
            # Experiment mode: apply_print_condition must run AFTER
            # _init_print_state, which hardcodes print_speed=2200.0 — calling
            # before would let that default clobber the injected value.
            if getattr(self, "config_mode_active", False):
                params = dict(
                    density=EXPERIMENT_DENSITY,
                    layer_passes=EXPERIMENT_LAYER_PASSES,
                    overfeed=EXPERIMENT_OVERFEED,
                    spread_speed=EXPERIMENT_SPREAD_SPEED,
                    print_speed=EXPERIMENT_PRINT_SPEED,
                    layer_thickness=EXPERIMENT_LAYER_THICKNESS,
                )
                applied = apply_print_condition(self, params)
                for key, want in (
                    ("applied_print_speed", params.get("print_speed")),
                    ("applied_spread_speed", params.get("spread_speed")),
                    ("applied_overfeed", params.get("overfeed")),
                    ("applied_layer_passes", params.get("layer_passes")),
                    ("applied_density", params.get("density")),
                ):
                    if want is not None and applied[key] != (int(want) if key in ("applied_layer_passes", "applied_density") else float(want)):
                        raise RuntimeError(
                            f"Experiment injection mismatch: {key} readback {applied[key]!r} "
                            f"!= requested {want!r}. Refusing to start print."
                        )
                print(f"[Experiment Print] {applied}")
            ...
            if start_layer == 1:
                print("--- Spreading initial powder layer ---")
                first_thickness = self.config_layer_thickness \
                    if getattr(self, "config_mode_active", False) \
                    else self.imageconverter.svg_layer_height[0]
                self.grbl.NewLayer(first_thickness)
                while self.grbl.nl_state == 0:
            ...
            while True:
                ...
                if getattr(self, "config_mode_active", False):
                    # DOE mode: spread thickness comes from DOE_LAYER_THICK_MM,
                    # not the SVG. current_layer_height stays SVG-derived so
                    # progress/remaining-height displays keep reading the
                    # file's own layer index (same rationale as the
                    # ConfigRunner branch in _print_single_config_layer).
                    temp_layer_thickness = self.config_layer_thickness
                else:
                    temp_layer_thickness = next_h - self.current_layer_height
